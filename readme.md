...
- `npm dev:frontend` - Start the frontend astro server on port 4321
- `npm dev:backend` - Start the backend axum server on port 3000

### Production Build

- `npm build` - Build the frontend astro site, and the backend axum server

### Run Production

- `npm start` - Start the production server on port 3000 serving the astro site and running the axum server for routes under `/api/`

### Linting & Formatting

- `npm lint` - Lint the rust code, web code linting coming soon
- `npm format` - Format the rust and web code

### Quick Launch

- `npm launch` - Build and start the production server
